...
function highlightMarker(marker) {
    ...
    if (currentHighlightedMarker && currentHighlightedMarker !== marker) {
        ...
        // 隐藏标签
        const prevLabel = currentHighlightedMarker.getLabel();
        if (prevLabel) {
            prevLabel.setStyle({
                display: 'none'
            });
    ...
    try {
        ...
        const highlightIcon = new window.BMap.Icon(
            '/static/images/marker-highlight.png',
    ...
    if (label) {
        label.setStyle({
            ...
            border: '2px solid #C1302E',
            borderRadius: '8px',
            padding: '5px 12px',
            boxShadow: '0 4px 15px rgba(193, 48, 46, 0.35)',
...
window.MapCore = {
            ...
            if (item.lat && item.lng) {
                ...
                if (!isNaN(lat) && !isNaN(lng)) {
                    ...
                    label.setStyle({
                        ...
                        borderRadius: '8px',
                        padding: '4px 10px',
                        boxShadow: '0 3px 10px rgba(193, 48, 46, 0.25)',
                        textShadow: '0 1px 1px rgba(255, 255, 255, 0.5)'
                    });
    ...
    addMapControls: function() {
        ...
        // 添加地图工具栏（使用工具栏模块）
        if (window.MapToolbar && typeof window.MapToolbar.create === 'function') {
            window.MapToolbar.create(map);
            window.MapToolbar.addTooltips();
        }
    },
...
// 立即检查是否已经初始化（处理脚本加载顺序问题）
if (window.mapCoreInitializing) {
    mapInitialized = true;
    console.log('检测到window.mapCoreInitializing，标记mapInitialized = true');
}

// 监听地图初始化开始事件（由heritage-map.html中的回调触发）
window.addEventListener('mapCoreInitializing', function() {
    mapInitialized = true;
    console.log('收到地图初始化开始事件，标记mapInitialized = true');
});

// 监听地图初始化完成事件
window.addEventListener('mapCoreInitialized', function() {
    console.log('收到地图初始化完成事件');
});

// 备用初始化：仅在百度地图API加载完成但回调未触发时使用
document.addEventListener('DOMContentLoaded', function() {
    // 如果地图已初始化，则不再重复初始化
    if (mapInitialized) {
        console.log('地图已初始化，跳过DOMContentLoaded中的初始化');
        return;
    }
    
    // 等待5秒，如果地图仍未初始化，则尝试初始化
    // 这是为了处理百度地图API加载完成但回调未触发的情况
    setTimeout(() => {
        // 再次检查全局标记
        if (window.mapCoreInitializing) {
            mapInitialized = true;
        ...
        if (!mapInitialized && typeof window.BMap !== 'undefined') {
            console.log('地图未初始化但BMap已加载，尝试备用初始化...');
            initMapWhenReady();
        }
    }, 5000);
});
...
function initMapWhenReady() {
    // 再次检查是否已初始化
    if (mapInitialized) {
        console.log('地图已初始化，跳过initMapWhenReady');
        return;
    }
