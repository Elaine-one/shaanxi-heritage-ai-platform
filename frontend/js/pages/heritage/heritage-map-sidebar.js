...
function updateCollectionButtonsState() {
    ...
    // 提取所有收藏的项目ID
    const collectedIds = collectionsArray.map(c => {
        if (typeof c === 'number') {
            return c;
    ...
    console.log('更新收藏按钮状态，已收藏ID列表:', collectedIds);
...
async function loadCollectionsFromStorage() {
    try {
        ...
        if (!user || !user.username) {
            // 用户未登录，清空收藏列表
            collections = [];
            console.log('用户未登录，收藏列表为空');
            return;
        }
        
        // 从后端API加载收藏数据
        if (window.heritageAPI && window.heritageAPI.getUserFavorites) {
            const favorites = await window.heritageAPI.getUserFavorites();
            collections = Array.isArray(favorites) ? favorites : [];
            console.log('已从后端加载收藏数据:', collections.length, '项');
        } else {
            collections = [];
        }
    } catch (error) {
...
function saveCollectionsToStorage() {
    // 数据已通过API保存到后端，这里只更新本地缓存用于快速显示
    console.log('收藏数据已保存到后端，本地缓存:', collections.length, '项');
}
    ...
    window.heritageAPI.checkFavoriteStatus(itemId)
        ...
        .catch(error => {
            ...
            if (error.message && error.message.includes('未登录')) {
                showToast('请先登录后再收藏');
                // 显示登录弹窗
                if (window.modalManager && window.modalManager.showLoginModal) {
