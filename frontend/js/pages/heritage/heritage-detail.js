...
function openLightbox(imageSrc, imageAlt) {
    console.log('openLightbox called:', imageSrc, imageAlt);
    ...
    allGalleryItems.forEach((item, index) => {
        ...
        if (img) {
            ...
            // 找到当前图片的索引 - 使用 includes 来匹配 URL
            if (imageSrc.includes(imgSrc) || imgSrc.includes(imageSrc) || img.src === imageSrc) {
                currentIndex = index;
            }
        }
    });

    console.log('Collected images:', allImages, 'currentIndex:', currentIndex);

    // 使用高级Lightbox组件
    if (typeof AdvancedLightbox !== 'undefined') {
        AdvancedLightbox.open(imageSrc, imageAlt, allImages, currentIndex);
    } else {
        // 回退到原有实现
        openLightboxFallback(imageSrc, imageAlt, allImages, currentIndex);
    }
}

// 原有Lightbox实现作为回退
function openLightboxFallback(imageSrc, imageAlt, allImages, currentIndex) {
