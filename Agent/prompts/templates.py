...
5. 不要重复调用同一工具：如果对话历史中已有相同工具+相同参数的调用结果，请直接使用已有结果。仅当参数确实不同或之前调用失败时才重新调用。
...
L1_TURN_SUMMARY_PROMPT = """你是一个对话摘要助手。请用一句话（不超过50字）概括以下对话片段中的关键信息。

要求：
1. 只提取实质决策、偏好变化、地点选择、非遗兴趣
2. 忽略寒暄、确认语句、工具调用细节
3. 不要重复对话中已明确的信息，只提炼核心要点
...
L1_TURN_SUMMARY_SYSTEM = "你是一个精准的对话摘要助手，只用一句话总结对话中的关键决策和偏好变化。"


def get_l1_turn_summary_prompt(content: str) -> str:
    """获取 L1 对话轮次摘要 Prompt"""
    return L1_TURN_SUMMARY_PROMPT.format(content=content)
...
def get_pdf_content_prompt(
    ...
    key_decisions_str: str = "",
    related_sessions_str: str = ""
) -> str:
